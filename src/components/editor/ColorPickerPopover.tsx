...
interface ColorPickerBodyProps {
  ...
  onRequestClose?: () => void;
  /** Avisa o contêiner que o seletor nativo de cor do sistema foi aberto. */
  onNativePickerOpen?: () => void;
}

/**
 * Corpo do seletor de cores (aba sólida + gradiente), reutilizado pelo popover
 * da barra de ferramentas e pelo painel flutuante "Cor do componente".
 */
export function ColorPickerBody({
  ...
  const [currentHex, setCurrentHex] = useState(
    initialColor.startsWith("#") ? initialColor : "#8b5cf6",
  );
      ...
      {activeTab === "solid" ? (
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="grid max-h-24 grid-cols-8 gap-1 overflow-y-auto pr-1">
              {SWATCH_COLORS.map((c) => (
                <button
                  key={c}
                  type="button"
                  ...
                  className="h-5 rounded border border-border/40 transition-transform hover:scale-105 hover:border-white"
                  style={{ background: c }}
                />
              ))}
            </div>
...
export function ColorPickerPopover({
  ...
  // Momento em que o seletor nativo de cor do sistema abriu; cliques fora do
  // popover feitos nessa janela nativa não devem fechá-lo por alguns segundos.
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {children ? (
          ...
          <button
            ...
            className="flex h-8 min-w-8 items-center justify-center gap-1 rounded px-2 text-xs font-semibold text-foreground/80 transition-colors hover:bg-secondary hover:text-foreground"
          >
            {label || (
      ...
      <PopoverContent
        align="start"
        className="w-80 border border-border bg-popover/95 p-3 shadow-2xl backdrop-blur z-50"
      ...
      >
        <ColorPickerBody
          showGradientTab={showGradientTab}
          initialColor={color.startsWith("#") ? color : "#8b5cf6"}
          onPick={onPick}
