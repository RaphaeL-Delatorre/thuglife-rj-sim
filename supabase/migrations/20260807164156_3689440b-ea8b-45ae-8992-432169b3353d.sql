...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = _user_id AND ('*' = ANY(r.permissions) OR _perm = ANY(r.permissions))
  )
$$;
...
CREATE POLICY "site_stats_public_read" ON public.site_stats FOR SELECT TO anon, authenticated USING (true);
...
CREATE POLICY "requirements_public_read" ON public.requirements FOR SELECT TO anon, authenticated USING (true);
...
CREATE POLICY "news_public_read" ON public.news FOR SELECT TO anon, authenticated USING (published);
...
GRANT SELECT ON public.rule_categories TO anon, authenticated;
GRANT ALL ON public.rule_categories TO service_role;
ALTER TABLE public.rule_categories ENABLE ROW LEVEL SECURITY;
CREATE POLICY "rule_categories_public_read" ON public.rule_categories FOR SELECT TO anon, authenticated USING (true);

CREATE TABLE public.rule_sections (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  block text NOT NULL DEFAULT 'termos',
  title text NOT NULL,
  sort_order integer NOT NULL DEFAULT 0
);
...
CREATE POLICY "rule_sections_public_read" ON public.rule_sections FOR SELECT TO anon, authenticated USING (true);
...
CREATE POLICY "rules_public_read" ON public.rules FOR SELECT TO anon, authenticated USING (true);

CREATE TABLE public.actions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  porte text NOT NULL DEFAULT '',
  nome text NOT NULL,
  bandidos integer NOT NULL DEFAULT 0,
  policia integer NOT NULL DEFAULT 0,
...
INSERT INTO public.roles (name, description, permissions, is_system) VALUES
  ('Desenvolvedor', 'Acesso total ao painel e ao site.', ARRAY['*'], true);
...
INSERT INTO public.site_settings (key, value) VALUES
  ('geral', jsonb_build_object(
    ...
    'heroSubtitle','escreve a História na temática Baque RJ há mais de 10 anos',
...
INSERT INTO public.news (tag,title,body,sort_order) VALUES
  ('Servidor','Novo sistema de rachas noturnos na Zona Sul','Circuitos fechados, apostas entre crews e recompensa por reputação. As pistas abrem toda sexta, 22h, com fiscalização policial dinâmica.',1),
  ('Atualização','Aeromóvel policial e novo protocolo de operações','As forças de segurança ganharam suporte aéreo, perseguição por rota e comunicação integrada entre batalhões dentro da cidade.',2),
  ('Evento','Verão TLRJ: temporada de eventos na orla','Quiosques jogáveis, campeonatos de futevôlei, shows ao vivo e empregos temporários exclusivos durante toda a temporada.',3);
...
INSERT INTO public.rule_categories (name,sort_order) VALUES
  ('Termos de Compras',1),('Regras Gerais',2),('Código Penal',3),('Áreas Safes',4),('Regras Policiais',5),
  ('Regras do Ilegal',6),('Regras do Hospital',7),('Regras de Denúncias',8),('Regras de Telagem',9),('Regras de Ações',10);
...
WITH s AS (
  INSERT INTO public.rule_sections (block,title,sort_order) VALUES
    ('termos','1. Acesso à Cidade',1),
    ('termos','2. Regras de Participação',2),
    ('termos','3. Discord Oficial',3),
    ('termos','4. Itens e Economia',4),
    ('termos','5. Wipe e Temporadas',5),
    ('termos','6. Atualizações e Revisões',6),
    ('gerais','Conduta e Roleplay',7),
    ('gerais','Regras Policiais',8),
    ('gerais','Regras do Ilegal',9),
    ('gerais','Hospital e Áreas Safe',10)
  RETURNING id, title
)
